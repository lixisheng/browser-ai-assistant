...
Browser AI Assistant 是一个基于 Chrome Manifest V3 的浏览器侧边栏 AI 助手。它可以读取当前标签页内容，把页面标题、链接、正文或 HTML 作为对话上下文，并通过用户自定义的模型渠道完成网页问答、总结、解释、改写、图片理解和聊天记录管理。

项目当前聚焦本地可控的 Chrome 扩展体验：模型、API Key、提取规则、聊天历史和同步配置都保存在浏览器本地 IndexedDB 中；同步功能仅在用户显式开启后才会把数据备份到 Chrome Sync、WebDAV 或 S3 兼容存储。
...
- 支持点击扩展图标打开侧边栏。
- 支持右键菜单“打开 AI 助手”。
- 支持快捷键打开侧边栏，默认 `Ctrl+Shift+Y`，macOS 为 `Command+Shift+Y`。
- Content Script 构建为普通 IIFE 脚本，适配 `manifest.json` 的 `content_scripts` 和运行时按需注入。
- 当页面缺少当前版本 Content Script 时，后台会按需注入并重试一次页面上下文提取。
...
- 支持把页面标题、当前 URL 和页面内容按稳定结构注入到对话上下文。
- 支持上下文查看弹窗和手动刷新。
- 发送前会按模型 `maxTokens` 预算裁剪页面上下文，避免过量保留页面内容。
- 规则命中但提取为空、选择器异常或 URL 未命中时，会回退到全局页面内容提取。
- 上下文刷新存在竞态保护，较早的慢速结果不会覆盖较新的提取模式结果。

### 提取规则

- 支持按 URL 正则配置页面提取规则。
- 每条规则支持多行 CSS 选择器或 XPath。
- 多条规则同时命中时按 `sortOrder` 顺序选择第一条。
- 支持新增、编辑、删除和上下移动规则。
- 当前页面命中的规则会在列表中高亮并顶置。
- 新增规则必须显式保存，非法 URL 正则、空选择器、非法 CSS/XPath 不会落库。
- 支持调用已配置模型为当前 URL 生成 5 个候选 JavaScript RegExp 正则，并可一键填入规则。
...
- 端点地址可只填写基础域名，代码会自动补全聊天、消息或模型列表路径，同时兼容历史上已保存的完整路径。
- 支持从渠道拉取远端模型列表、搜索模型、添加远端模型。
- 支持手动新增模型。
- 支持模型连通性测试，并以单模型粒度展示测试状态。
- 支持删除单个模型或删除整个渠道；删除渠道会同步清理其下模型。
- 支持设置全局默认对话模型。
- 支持为每个会话持久化当前选择的模型，切换历史会话时恢复该会话的模型。
- 支持设置全局唯一的 AI 标题生成模型。
- 支持标记模型是否支持视觉理解；支持视觉的模型会在模型列表、默认模型选择、标题模型选择和当前模型选择中显示视觉标识。
...
- 若流式连接未收到任何响应即断开，会自动回退到非流式请求。
- 支持解析 `<think>...</think>` 思考过程，并在消息中以可折叠区域展示。
- 支持 OpenAI 兼容流式返回中的 `reasoning_content` 思考片段。
- 支持 Markdown 渲染、GFM 表格、代码块、行内代码、有序列表和无序列表。
- 支持用户消息与 AI 消息重新生成；重新生成会丢弃该消息及后续消息，再用上方用户消息重新请求。
- 支持编辑历史用户消息后重新生成，并保留该用户消息已有图片附件。
- 支持请求中附带完整会话历史。
- 支持失败提示，模型请求异常不会保存失败的 AI 消息。
- 支持发送中继续输入，响应完成不会清空新输入草稿。
- 支持防重复发送，快速连续发送只会保留第一次有效请求。
- 支持可配置发送快捷键：`Enter`、`Shift+Enter`、`Ctrl+Enter`、`Ctrl+Shift+Enter`、`Alt+Enter`。
- 中文输入法组合输入期间不会触发快捷键发送。
- 支持开关是否把页面上下文拼接到 system prompt。
- 支持按会话覆盖系统提示词、temperature、max tokens 和 top_k。
- 支持全局聊天偏好：系统提示词、temperature、max tokens、top_k、发送快捷键、历史面板默认展开状态。
...
- 支持图片缩略图、删除和放大预览。
- 支持发送纯图片消息。
- 支持历史用户消息中的图片持久化、重新加载后展示和放大预览。
- 当前模型不支持视觉理解时会禁用图片输入，并隐藏截图入口。
- 图片类型限制为 PNG、JPEG、WebP、GIF。
...
- 支持新建会话。
- 支持会话重命名、归档、删除，删除需要二次确认。
- 支持历史会话文件夹。
- 支持新建文件夹、重命名文件夹。
- 支持把未归档会话拖拽到文件夹，或拖回默认文件夹。
- 支持默认文件夹、自定义文件夹和已归档分区。
- 自定义文件夹默认折叠，可手动展开。
- 宽面板下支持左侧历史面板展开/折叠。
- 窄面板下支持通过历史抽屉浏览和管理历史记录。
- 长标题会截断，避免撑出横向滚动。
- 首轮对话完成后可使用配置的标题模型生成会话标题；标题模型失败、返回非 JSON 或空标题时保留兜底标题。

### 隐私模式

- 空白会话可进入隐私模式。
- 隐私模式中的消息只保存在内存，不会自动持久化为历史会话。
- 隐私模式支持普通响应和流式响应。
- 可以手动保存隐私会话，保存后退出隐私模式并成为普通历史会话。
- 保存隐私会话后可基于完整聊天记录重新生成标题。
...
- 支持导出当前聊天为 Word 文档。
- 支持通过浏览器打印窗口导出 PDF。
- 导出内容包含标题、导出时间、会话创建时间、会话更新时间、消息数量和全部消息。
- 导出会保留 system、user、assistant 角色和思考过程。
- Markdown 导出会处理正文中已有代码围栏，避免外层代码块提前截断。
- 下载文件名会清理非法字符、空标题和前导点。
- Markdown 下载后会释放 Blob URL，避免侧边栏长时间打开时累积内存。

### 数据同步与备份

- 支持手动备份和手动恢复。
- 支持自动同步定时任务，浏览器安装、启动和闹钟触发时恢复或执行备份。
- 支持 Chrome Sync。
- 支持 WebDAV，并在远程目录不存在时按层级创建目录后重试写入。
- 支持 S3 兼容存储，使用 AWS Signature V4 生成请求签名，采用 path-style URL。
- 支持备份前缀，前缀为空、包含路径分隔符或连续点号时会拒绝备份。
- 支持可选 AES-GCM 加密，密钥通过 PBKDF2 派生。
- 加密开启但未设置本地密钥时拒绝备份。
- 恢复加密备份时密钥错误会返回明确错误。
- 同步快照包含模型、渠道、提取规则、聊天会话、文件夹和应用设置。
- 同步快照会过滤本地密钥类设置：加密密钥、WebDAV 密码、S3 Secret Key。
- 恢复远端快照会覆盖本地业务数据，但保留当前本地同步密钥和远程凭据。
- Chrome Sync 备份会检查单项配额，超限时提示改用 WebDAV/S3 或减少历史记录。
- 手动恢复需要二次确认，避免误覆盖本地数据。

### 界面与交互

- 侧边栏使用暖奶油画布、珊瑚主色和深色产品面板的 Claude 风格主题。
- 主题令牌集中在 `src/side-panel/themes/claude-light.css`，公共组件样式集中在 `src/side-panel/styles.css`。
- 设置页使用设置级 Tab：渠道管理、提取规则、聊天偏好、同步设置、界面偏好。
- 输入框、密码框、系统提示词、备份前缀、WebDAV/S3 配置等文本控件兼容中文输入法组合输入。
- 聊天主区域固定在面板内，消息列表内部滚动。
- 代码块和长 URL 在局部容器内滚动或换行，避免撑出侧边栏。
- 圆形图标按钮、图片删除按钮、抽屉关闭按钮均设置明确的可访问标签。
...
6. 可选：测试模型连通性。
7. 可选：设置默认对话模型、AI 标题生成模型、视觉理解能力。
8. 回到聊天页，刷新当前页上下文后开始对话。
...
本地数据保存在 IndexedDB 数据库 `browser-ai-assistant` 中，当前数据库版本为 `2`。
...
- OpenAI 兼容与 Anthropic 请求构造、模型列表、连通性测试。
...
项目处于快速迭代阶段，已具备可用的网页上下文聊天、模型渠道管理、提取规则、视觉输入、历史会话、隐私模式、多格式导出和数据同步能力。后续仍可继续完善更多模型协议、更多浏览器端兼容性验证、同步冲突策略和更完整的端到端测试覆盖。
