...
* 第一阶段只允许搭建权限、开关、风险提示和连接生命周期；第二阶段只允许接入 `browser.take_snapshot` 快照闭环；第三阶段只允许接入 `browser.click`、`browser.fill`、`browser.press_key`、`browser.wait_for` 基础操作；导航、多页面控制和高风险工具必须按规划继续分阶段接入。
