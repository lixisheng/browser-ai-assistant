...
完整 Web 逆向能力继续按阶段推进。Source map 已作为独立 `sourcemap.*` 工具组落地，受控只读 `Runtime.evaluate` 已并入默认普通模式，用于补足静态 JS / Source Map 无法确认运行时模块、公开配置和函数导出形态的场景。本阶段同时落地三模式运行态、受控增强 AI 边界确认和无凭据请求重放沙箱 v1；敏感字段解锁与“完全访问”仍作为更高风险能力保持 fail closed，必须经过单独权限设计、用户确认、脱敏、审计和测试后才能启用。
...
- 完全访问模式仅作为状态和 UI 占位；当前 v1 不暴露无限制执行器，background 必须 fail closed。
...
1. 将阶段四运行时只读并入三模式运行态：普通模式默认开启受限只读，受控增强继承只读能力，完全访问占位 fail closed。
...
允许用户在明确知情的情况下，临时解锁某些敏感字段，用于本轮分析或一次请求重放。在此基础上，规划一个更高风险的“完全访问”授权层，用于用户主动承担风险并要求扩展在本轮诊断中读取更完整的页面、请求和运行时信息。

状态：仅纳入规划，不进入当前阶段实现；当前实现遇到 `full_access_reserved` 或等价授权上下文仍必须固定拒绝。
...
“完全访问”是最高风险临时授权，用于满足用户明确要求扩展读取完整请求、响应、运行时对象、页面状态或执行更高权限诊断动作的场景。它不是默认模式，也不是普通浏览器控制、只读 Runtime 或请求重放沙箱的自然升级；必须由用户在清晰风险提示后逐次开启。

用户期望能力包括：

- 临时关闭默认脱敏逻辑，让用户能看到本轮分析需要的原始字段。
- 临时关闭只读限制，允许执行用户明确批准的诊断脚本或操作。
- 临时关闭敏感信息过滤，让用户能在本地查看 Cookie、Authorization、Token、Storage 或请求/响应原文。

不可取消的底线：

- 不能把“完全访问”设计成模型或脚本不受任何限制；用户授权也不能让模型获得自我扩权、自我持久化、自我传播或绕过用户确认的能力。
- 不能自动执行破坏性、高频、批量、绕过验证码、绕过风控、撞库、爆破、权限提升、隐蔽采集或跨站批量扫描行为。
- 不能把敏感原文写入聊天长期历史、IndexedDB、同步快照、导出文件、远端模型上下文或工具附件，除非该具体输出也经过用户逐项确认且标记为本地临时查看。
- 不能关闭审计、取消机制、超时、大小上限、并发上限、目标范围校验和用户可见风险提示。
- 不能让模型直接传入任意长期运行脚本；高权限脚本必须由用户可见、可审查、可复制核对的草案进入确认流程。

建议授权模型：

- `full_access_reserved` 继续作为内部授权上下文占位；正式落地前所有执行器遇到该授权必须 fail closed。
- 完全访问必须是运行态临时授权，不进入聊天偏好、会话历史、同步快照或导出。
- 授权必须绑定当前 tab、当前 origin、当前会话轮次和短时效，默认在关闭浏览器控制、切换 tab、导航、刷新、debugger detach、生成终止或授权过期时立即失效。
- 开启前必须展示固定中文高危提示，明确说明可能读取敏感凭据、执行页面脚本、发起网络请求、改变页面状态和触发站点风控。
- 用户必须以逐次确认开启，不能通过模型工具调用、Prompt、历史消息、导入数据或同步快照自动开启。
- UI 必须提供醒目的关闭入口和当前授权状态；background 必须广播授权关闭事件，确保多个 Side Panel 状态同步回收。

建议工具分层：

| 层级 | 能力 | 默认状态 |
| --- | --- | --- |
| 细粒度敏感字段解锁 | 本轮查看某个 Cookie、Authorization 或响应字段原文 | 关闭，逐字段确认 |
| 完全访问读 | 本轮读取完整请求/响应、Storage 摘要或运行时对象原文 | 关闭，逐次确认 |
| 完全访问执行 | 本轮执行用户审查过的脚本或携带凭据请求 | 关闭，逐脚本/逐请求确认 |

建议工具：

| 工具 | 用途 |
| --- | --- |
| `full_access.prepare_action` | 生成高权限动作草案，包含脚本、请求、目标、敏感字段、可能副作用和回滚说明，不执行。 |
| `full_access.execute_action` | 仅执行用户本次确认过的草案，并返回本地临时结果摘要。 |
| `full_access.revoke` | 立即撤销完全访问授权、清理临时草案、取消进行中的高权限动作。 |

命名规则：

- 用户可见文档保留 `full_access.*` 点号名称。
- 发给 OpenAI-compatible 模型的 `function.name` 必须使用 `full_access_prepare_action`、`full_access_execute_action`、`full_access_revoke` 这类安全名称。
- `full_access.*` 属于最高风险浏览器自动化工具组，必须同时满足浏览器控制开启、完全访问临时授权、当前草案确认和 background 二次校验。

数据处理规则：

- 完全访问结果默认只在本地临时面板展示，不自动进入聊天消息正文。
- 如果用户明确要求把敏感原文写入聊天，必须二次确认，并在消息上标记“包含用户授权敏感信息”；导出和同步仍默认排除。
- 工具记录默认只保存动作类型、目标摘要、确认时间、执行状态、截断标记和是否包含敏感结果，不保存敏感原文。
- 发送给远端模型的上下文仍必须默认脱敏；如用户要求把敏感原文发送给模型，必须作为独立授权项确认，并提示第三方模型提供商可能接收这些数据。
- 完全访问产生的数据不得进入自动备份、Chrome Sync、WebDAV、S3 同步快照或默认导出。

执行边界：

- 高权限动作必须拆成“草案生成”和“确认执行”两步；草案必须展示可读脚本或请求摘要、目标 origin、敏感字段、预期副作用、超时和取消方式。
- 执行脚本必须有固定超时、最大输出大小、最大对象深度、最大网络请求次数和可取消机制。
- 请求类动作必须显示目标 URL、method、header 原文或摘要、body 原文或摘要、凭据携带情况和响应读取上限。
- DOM 写入、表单提交、点击、导航、下载、上传、删除、支付、下单、发帖、发送消息、修改账号设置等明显副作用动作必须逐项确认，不能批量执行。
- 对未知脚本、混淆脚本、长脚本、跨 origin 脚本、远程加载脚本、动态 `eval` / `Function`、无限循环迹象和批量请求迹象必须默认拒绝或要求用户手工审查后重新确认。

建议实施顺序：

1. 先实现细粒度敏感字段解锁的本地临时查看，不改变请求重放和 Runtime 默认脱敏策略。
2. 新增完全访问授权状态、风险提示、短时效、撤销、广播同步和生命周期清理，但所有执行器仍保持拒绝。
3. 实现 `full_access.prepare_action`，只生成高权限动作草案和风险摘要，不执行。
4. 实现最小 `full_access.execute_action`，先只支持用户确认的单次读取类动作，禁止写操作和网络请求。
5. 在完成审计、取消、敏感原文本地临时展示和测试后，再评估是否允许逐次确认的写操作或携带凭据请求。

验收标准：

- 未开启完全访问授权时，模型看不到 `full_access.*` 工具 schema，background 也拒绝伪造工具调用。
- 完全访问授权不能持久化，刷新、导航、切 tab、关闭浏览器控制、debugger detach、生成终止或超时后必须失效。
- 模型无法绕过草案和用户确认直接执行高权限动作。
- 敏感原文默认不进入聊天历史、同步快照、导出、工具附件或远端模型上下文。
- 用户能一键撤销授权并中止正在执行的高权限动作。
- 所有高权限失败都返回固定中文错误，不透出第三方敏感堆栈或 Chrome 原始调试报文。
